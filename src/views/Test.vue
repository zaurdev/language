        ...
        <div class="test-container">
            <ul class="word-list">
                <li v-for="(el, index) in getTest.arrValues" :key="index" :class="{ 'active': first == el.id }">
                    <a href="#" @click.prevent="setFirst(el, index)" :ref="`first-item-${el.id}`">
                        {{ el.equal }}
                    </a>
                </li>
            </ul>
            <ul class="word-list">
                <li v-for="(el, index) in getTest.arrKeys" :key="index" :class="{ 'active': second == el.id }">
                    <a href="#" @click.prevent="setSecond(el, index)" :ref="`second-item-${el.id}`">
                        {{ el.name }}
                    </a>
                </li>
            </ul>
        ...
        setSecond(el, index) {
            if (this.first) {
                ...
                Object.keys(this.$refs).forEach(e => {
                    let el = this.$refs[e];
                    if(el[0].classList.contains('active')){
                        el[0].classList.remove('active');
                    }
                    this.$refs[e] = el;
                });
                let secondEl = this.$refs['second-item-' + el.id];
                if (secondEl[0].classList.length == 0 || !secondEl[0].classList.contains('disabled')) {
                    this.second = el.id;
                    secondEl[0].classList.add('active');
                }
            }
        },
        setFirst(el, index) {
